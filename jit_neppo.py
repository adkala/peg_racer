...
def build_env_functions(
    ...
    def jax_reset(key: jax.Array) -> Tuple[EnvState, jnp.ndarray]:
        ...
        def car_features(i, _):
            obs5 = jnp.array(
                [cars.x[i], cars.y[i], cars.psi[i], cars.vx[i], cars.vy[i]]
            )
            tgt, _, s, e = wp_generate(obs5, cars.vx[i])
            theta = tgt[0, 2]
            theta_diff = jnp.arctan2(
                jnp.sin(theta - cars.psi[i]), jnp.cos(theta - cars.psi[i])
            )
            curv = tgt[0, 3]
            curv_lh = tgt[-1, 3]
            return 0, jnp.array(
    ...
    def jax_step(state: EnvState, action: jnp.ndarray):
        ...
        a0 = jnp.stack(
            [jnp.clip(a[:, 0], 0.0, 1.0), jnp.clip(a[:, 1], -1.0, 1.0)], axis=1
        )
        ...
        obs_before = jax.vmap(lambda i: obs_from_features(feats_before, i))(
            jnp.arange(3)
        )
        ...
        state2 = EnvState(
            cars=cars2, delay_buf=buf1, t=t2, last_rel=rel_after, track_L=track_L
        )
